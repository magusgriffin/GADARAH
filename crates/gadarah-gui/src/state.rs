...
pub struct Alert {
    ...
    /// Full Oracle advice body when this alert was generated by the auto-
    /// prompt path. The banner renders an "Open in Oracle" button when
    /// this is `Some`, which routes the user to the Oracle tab and shows
    /// the full text.
    #[serde(default)]
    pub oracle_advice: Option<String>,
    /// When true, the external dispatcher skips the OS notification for
    /// this alert. Used by auto-prompt follow-ups so the user doesn't get
    /// pinged twice for the same event (once for the trigger, once for
    /// the Oracle's take).
    #[serde(default)]
    pub suppress_os_notification: bool,
}
...
pub struct SharedState {
    ...
    /// Set by the alert banner when the user clicks "Open in Oracle" on an
    /// auto-prompt follow-up. The Oracle panel reads + clears this on its
    /// next render to display the full advice in the response area.
    pub pending_oracle_review: Option<String>,
}
